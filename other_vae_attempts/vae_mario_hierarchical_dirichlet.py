"""
Learning one hierarchical dirichlet from which we sample.
"""
...
class VAEMarioHierarchicalDirichlet(nn.Module):
    def __init__(
        self,
        w: int = 14,
        h: int = 14,
        ...
        super(VAEMarioHierarchicalDirichlet, self).__init__()
        self.w = w
        self.h = h
    ...
    def _intermediate_distribution(self, z: Tensor) -> Dirichlet:
        """
        Returns Dir(dec(z))
        """
        result = self.decoder(z.to(self.device))
        concentrations = self.dec_concentrations(result) + 1e-10

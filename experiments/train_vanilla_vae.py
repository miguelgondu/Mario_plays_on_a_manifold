...
def run(
    ...
    for epoch in range(max_epochs):
        ...
        if test_loss < best_loss:
            ...
            torch.save(vae.state_dict(), MODELS_PATH / f"{comment}_final.pt")
        else:
            if not overfit:
        ...
        if save_every is not None and epoch % save_every == 0 and epoch != 0:
            ...
            torch.save(vae.state_dict(), MODELS_PATH / f"{comment}_epoch_{epoch}.pt")
    ...
    print("Saving metadata alongside the model")
    with open(MODELS_PATH / f"{comment}_mariovae_final_metadata.txt", "w") as fp:
        fp.writelines(
